...
} from "@/lib/data-sources/handlers/scrape-facebook/config";
import {
  SOURCE_RUN_TYPE_FACEBOOK_COMMENTS,
  SOURCE_RUN_TYPE_FACEBOOK_POST,
  SOURCE_RUN_TYPE_FACEBOOK_POSTS,
} from "@/lib/data-sources/source-run-types";
import { scheduleFacebookCommentScrape } from "@/lib/data-sources/services/schedule-facebook-comment-scrape";
import { mapCommentToUpsertItem } from "@/lib/data-sources/handlers/scrape-facebook/utils/map-comment-to-upsert-item";
import { mapPostToDocument } from "@/lib/data-sources/handlers/scrape-facebook/utils/map-post-to-document";
import { parseFacebookComments } from "@/lib/data-sources/handlers/scrape-facebook/utils/parse-facebook-comment";
import { parseFacebookPosts } from "@/lib/data-sources/handlers/scrape-facebook/utils/parse-facebook-post";

type HandleBrightDataWebhookParams = {
  sourceRunId: string;
  payload: unknown;
};
...
async function upsertFacebookPostDocuments(
  ...
  await Promise.all(
    posts.map(async (post) => {
      const params = mapPostToDocument({ sourceOriginKey, post });
      const { documentId, outcome } = await upsertDocument(
        { ...params, sourceRunId, dataSourceId },
        workspaceId,
      );
      summary[outcome]++;
      if (outcome === "inserted") {
...
export async function handleBrightDataWebhook(
  params: HandleBrightDataWebhookParams,
): Promise<{ ok: true }> {
  ...
  const [run] = await db
    .select({
      ...
      dataSourceId: sourceRuns.dataSourceId,
      result: sourceRuns.result,
      workspaceId: dataSources.workspaceId,
      sourceType: dataSources.sourceType,
      jobParams: dataSources.params,
    })
    .from(sourceRuns)
    .innerJoin(dataSources, eq(sourceRuns.dataSourceId, dataSources.id))
    .where(eq(sourceRuns.id, params.sourceRunId))
    .limit(1);

  if (!run) {
    throw new NotFoundError("job run", params.sourceRunId);
  }
  ...
  if (
    run.runType === SOURCE_RUN_TYPE_FACEBOOK_COMMENTS &&
    ...
    (run.runType === SOURCE_RUN_TYPE_FACEBOOK_POSTS ||
      run.runType === SOURCE_RUN_TYPE_FACEBOOK_POST) &&
    Array.isArray(rawOutput)
  ) {
    const facebookUrl =
