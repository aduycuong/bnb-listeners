...
import type {
  GetTopicChartParams,
  GetTopicChartResult,
  TopicDetailChartDigest,
  TopicDetailChartPoint,
} from "../types";
import type { TopicDetailChartMetric } from "../topic-detail-chart-config";
import {
  resolveTopicDetailChartBucket,
  resolveTopicDetailChartPeriod,
  type TopicDetailChartBucket,
} from "../utils/resolve-topic-detail-chart-period";
...
function getMetricSelect(metric: TopicDetailChartMetric) {
  switch (metric) {
    case "doc_count":
...
function toDigest(row: DigestRow): TopicDetailChartDigest {
  return {
...
export async function getTopicChart(
  params: GetTopicChartParams,
  ctx: WorkspaceContext,
): Promise<GetTopicChartResult> {
  await assertTopicInWorkspace(params.id, ctx.workspaceId);

  const period = resolveTopicDetailChartPeriod({
    preset: params.period,
    startDate: params.startDate,
    endDate: params.endDate,
  });
  const chartBucket = resolveTopicDetailChartBucket(
    params.period,
    period.startDate,
    period.endDate,
  );
  ...
  const digestResult = await db.execute<DigestRow>(sql`
    ...
    WHERE tdd.topic_id = ${params.id}::uuid
