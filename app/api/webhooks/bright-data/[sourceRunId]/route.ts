...
import { handleBrightDataWebhook } from "@/lib/data-sources/services/handle-bright-data-webhook";
import { z } from "zod";
...
export async function POST(
  ...
  try {
    ...
    const { sourceRunId } = routeParamsSchema.parse(await context.params);
    const payload = await request.json();

    await handleBrightDataWebhook({ sourceRunId, payload });
