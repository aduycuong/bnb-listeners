...
Admin maintains the topic list. Topics are the only classification dimension — industry, news group, and entity distinctions are expressed as topics.
...
1. After scoring, an LLM classifier reads each item and selects matching topics from the full topic list (including unverified LLM-created topics).
2. If one or more topics match, the item is assigned immediately with a confidence score.
3. If no topic matches, the LLM proposes a new topic (name + description), creates it (`verified = false`), and assigns the document.
