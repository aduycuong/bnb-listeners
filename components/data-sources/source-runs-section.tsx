...
import type { ListSourceRunsResult } from "@/lib/data-sources/types";
import { cn } from "@/lib/utils";
import { workspaceFetch } from "@/lib/workspaces/utils/workspace-fetch";

type SourceRunsSectionProps = {
  workspaceId: string;
  dataSourceId: string;
  canRun?: boolean;
};
...
async function fetchSourceRuns(
  ...
  const res = await workspaceFetch(workspaceId, `/api/data-sources/${dataSourceId}/runs`);
  const data = (await res.json()) as ListSourceRunsResult & {
    error?: string;
    message?: string;
  };

  if (!res.ok) {
    throw new Error(data.message ?? data.error ?? "Could not load dataSource runs.");
  }
...
export function SourceRunsSection({
  ...
  dataSourceId,
  canRun = false,
}: SourceRunsSectionProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: sourceRunsQueryKey(workspaceId, dataSourceId),
    queryFn: () => fetchSourceRuns(workspaceId, dataSourceId),
  });
      ...
      <CardContent>
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">
            No runs yet. Runs appear here after you run the dataSource or the schedule
            fires.
          </p>
        ) : (
