...
import {
  TOPIC_DETAIL_CHART_METRIC_LABELS,
  TOPIC_DETAIL_CHART_METRICS,
  TOPIC_DETAIL_CHART_PERIOD_LABELS,
  TOPIC_DETAIL_CHART_PERIOD_PRESETS,
  type TopicDetailChartMetric,
  type TopicDetailChartPeriodPreset,
} from "@/lib/topics/topic-detail-chart-config";
import type { GetTopicChartResult } from "@/lib/topics/types";

const ROLLING_PERIODS: TopicDetailChartPeriodPreset[] = [
  "last_7_days",
  "last_30_days",
  "last_90_days",
  "last_12_months",
...
const CALENDAR_PERIODS: TopicDetailChartPeriodPreset[] = [
  "this_week",
  "last_week",
  "this_month",
  "last_month",
...
type TopicDetailChartSectionProps = {
  period: TopicDetailChartPeriodPreset;
  metric: TopicDetailChartMetric;
  customStartDate?: string;
  customEndDate?: string;
  chart?: GetTopicChartResult;
  isLoading?: boolean;
  errorMessage?: string;
  onPeriodChange: (period: TopicDetailChartPeriodPreset) => void;
  onMetricChange: (metric: TopicDetailChartMetric) => void;
...
function formatMetricValue(metric: TopicDetailChartMetric, value: number | null) {
  if (value === null) {
    return "—";
  }
...
export function TopicDetailChartSection({
  period,
  metric,
  customStartDate,
  customEndDate,
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <DropdownMenuContent align="end" className="min-w-48">
                  <DropdownMenuGroup>
                    ...
                    <DropdownMenuRadioGroup
                      value={period}
                      onValueChange={(value) => {
                        const next = value as TopicDetailChartPeriodPreset;
                        if (next === "custom") {
                          setCustomOpen(true);
                          return;
                        }
              ...
              <DropdownMenu>
                ...
                >
                  {TOPIC_DETAIL_CHART_METRIC_LABELS[metric]}
                  <ChevronDownIcon className="size-4 text-muted-foreground" />
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="min-w-44">
                  <DropdownMenuRadioGroup
                    value={metric}
                    onValueChange={(value) =>
                      onMetricChange(value as TopicDetailChartMetric)
                    }
                  >
                    {TOPIC_DETAIL_CHART_METRICS.map((option) => (
                      <DropdownMenuRadioItem key={option} value={option}>
                        {TOPIC_DETAIL_CHART_METRIC_LABELS[option]}
                      </DropdownMenuRadioItem>
                    ))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
